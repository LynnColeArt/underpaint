...
| HQ-SAM | Sharper SAM-family segmentation backend for object decomposition and future layer extraction presets. | Model weights download to `~/.underpaint/models/segmentation/sam-hq-vit-base/`; no source or weights are committed to this repo. The worker uses Transformers `SamHQModel` / `SamHQProcessor`. | Apache-2.0 per model card/repo. Verify exact checkpoint terms before bundling. |
| BiRefNet | High-quality dichotomous segmentation and matting candidate for cleaner foreground alpha edges, especially hair, fur, fabric, petals, and soft boundaries. | Model weights download to `~/.underpaint/models/segmentation/birefnet/` via `tools/ai/download-underpaint-birefnet.sh`; no source or weights are committed to this repo. The bakeoff harness loads it through Transformers remote code. | MIT per Hugging Face model card. Remote-code model loading should stay isolated to the out-of-process AI worker/harness. |
| BEN2 Base | Background removal and edge-refinement candidate with a separate `ben2` runtime package. | Tracked in the model registry as research only. No runtime or weights are committed. | MIT per Hugging Face model card. Review package/runtime shape before wiring. |
| PhotoGIMP | Design reference for a familiar photo-editing workspace: compact tool rail, curated visible tools, color placement, dock layout, and shortcut/menu direction. | Reference only. A local evaluation clone was used under `/tmp`; no PhotoGIMP source, config, branding, icons, or splash assets are copied into this repo. | GPL-3.0. Compatible with Underpaint's GPL direction, but exact copied files or assets would need upstream file, commit, license, attribution, and destination recorded. |
